...
from scipy.stats.stats import chisquare, sem

from data_profiling.src.version import __version__
from data_profiling.src.config import config as config
from data_profiling.src.model.messages import (
...
from data_profiling.src.model import base
from data_profiling.src.model.base import Variable
from data_profiling.src.model.correlations import calculate_correlations
from data_profiling.src.visualisation.missing import (
    missing_bar,
    missing_matrix,
    missing_heatmap,
    missing_dendrogram,
)
from data_profiling.src.visualisation.plot import scatter_pairwise, make_pie_chart
...
def get_pie_chart(df, variables) -> list:
    disable_progress_bar = not config["progress_bar"].get(bool)

    # TODO: Add logic to ignore pie chart for fields with high cardinality
    cat_variables = [
        column for column, type in variables.items() if type == Variable.TYPE_CAT
    ]
    with tqdm(
        total=len(cat_variables),
        desc="pie charts",
        disable=disable_progress_bar,
    ) as pbar:
        
        pie_plots = {x: x for x in cat_variables}
        for x in cat_variables:
                pie_plots[x] = make_pie_chart(df[x])
                pbar.update()
...
def describe(df: pd.DataFrame) -> dict:
    ...
    # TODO: Add back once high cardinality fixed
    # Get pie charts
    # pie_charts = get_pie_chart(df, variables)
